...
-(void)configTableDate{
    ...
    for (NSString *key  in self.sectionTitleArry) {
        ...
        //通过便利字典中的key值取出对应的valew值，，，取出来为   德日美中国家
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSMutableArray *group=self.allCarArray[indexPath.section];
    CarDateModel *model=group[indexPath.row];
    NSLog(@"%@",group[indexPath.row]);
    NSLog(@"%@",model);
    
    //通过每一行cell显示model的数据来获取cell的高度
    CGFloat cellHeight=[CustomTableViewCell getCellHeightWithDateModel:model];

...
-(NSMutableArray *)selectAllLinkmans{
    ...
    char * error=nil;
    int result=sqlite3_exec(datebace, [sql UTF8String], NULL, NULL, &error);
    ...
    if (result == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt)== SQLITE_ROW) {
            ...
            NSString *remarks=[NSString stringWithUTF8String:(const char *)sqlite3_column_text(stmt, 5)];
            
            
            
            LinkMan *lingkman=[LinkMan linkManWithName:name andPhoneNumber:phoneNumber gender:gender age:age remarks:remarks];
...
//    NSString *sql=@"selete name,age,phoneNumber  from LinkMans where gender = ? ";
...
//            NSString *Name=[NSString stringWithUTF8String:(const char *)name];
            ...
            //把取出的数据封装到Linkmans，再把linkMans存放到数组里
            
            LinkMan *linkMan=[LinkMan linkManWithName:name andPhoneNumber:phoneNumber gender:gender age:age remarks:remarks];

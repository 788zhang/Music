...
-(void)canselWay{
   MangoViewController  *view = [[MangoViewController alloc]init];
    [self presentViewController:view animated:YES completion:nil];
}
//注册成功
-(void)registerSucceedWay{
    DataBaseManger *database = [[DataBaseManger alloc]init];
    LinkMan *man = [[LinkMan alloc]initWithName:self.nameKuang.text phoneNumber:self.phoneNumKuang.text gender:self.genderKuang.text age:18 remarks:self.remarksKuang.text];
    ;
    
    if ([database insertIntoNewLinkMan:man] == 1) {
        UIAlertView *alter = [[UIAlertView alloc]initWithTitle:@"提示" message:@"添加成功" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        [alter show];
    }else{

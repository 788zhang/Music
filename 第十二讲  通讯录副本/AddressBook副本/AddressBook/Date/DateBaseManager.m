...
//    NSString *sql=@"CREATE  TABLE LinkMans(number INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL,age INTEGER DEFAULT 18, phoneNumber TEXT NOT NULL, remarks TEXT)";
    ...
    NSString *sql=@"CREATE TABLE LinkMans(number integer primary key autoincrement, name text not null,gender text not null,age text not null,phoneNumber text not null,remarks text,imageName text)";
...
-(void)insertIntoLinkMans:(AdressBookDateModel *)linkman{
    ...
    NSString *sql=@"INSERT INTO LinkMans(name,gender,age,phoneNumber, remarks, imageName)values(?,?,?,?,?,?)";
    ...
    if (result == SQLITE_OK) {
        ...
        sqlite3_bind_text(stmt, 6, [linkman.imageName UTF8String], -1, NULL);
...
-(void)deleteLinkMan{
    ...
    NSString *path=@"delete from  LinkMans";
...
-(NSMutableArray *)selectAllLinkmans{
    ...
    if (result == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            const unsigned char *imageName =sqlite3_column_text(stmt,6);
            //把列里的数据再进行类型的转换
            ...
            NSString *imagename=[NSString stringWithUTF8String:(const char *)imageName];
